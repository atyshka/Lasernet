...
ds = tf.data.experimental.load('/home/alex/dataset-drive/interleaved_ds', (
                            ...
                            tf.TensorSpec(shape=None, dtype=tf.float32)), compression='GZIP', reader_func=custom_reader_func)

for i in range(20):
    shard = ds.take(7904)
    shard = shard.shuffle(7904)
    tf.data.experimental.save(shard, '/home/alex/dataset-drive/ds_sharded/shard_%d'%i, compression='GZIP', shard_func=lambda w,x,y,z: tf.constant(i, dtype=tf.int64))
    ds = ds.skip(7904)
