...
class ResnetBlock(layers.Layer):
  def __init__(self, num_filters, kernel_size=(3, 3), strides=(1, 1), reshape=False, name=None, dtype=None):
    ...
    self.conv1 = layers.Conv2D(num_filters, kernel_size, kernel_initializer='he_normal', strides=strides, use_bias=False, padding='same', dtype=dtype)
    self.bn1 = layers.BatchNormalization(dtype=dtype)

    self.conv2 = layers.Conv2D(num_filters, kernel_size, kernel_initializer='he_normal', use_bias=False, padding='same', dtype=dtype)
    ...
    if reshape or strides != (1, 1):
        ...
        self.conv3 = layers.Conv2D(filters=num_filters, kernel_size=(1, 1), kernel_initializer='he_normal', strides=strides, use_bias=False, dtype=dtype)
...
class FeatureExtractor(layers.Layer):
    def __init__(self, num_filters, num_blocks=7, downsample=True, reshape=False, dtype=None, name='FeatureExtractor'):
        super(FeatureExtractor, self).__init__(name=name, dtype=dtype)

        self.sequence = keras.Sequential()
        # Downsample by 2 along horizontal
        if downsample:
            self.sequence.add(ResnetBlock(num_filters, strides=(1, 2), name="Downsample", reshape=True, dtype=dtype))
        else:
            self.sequence.add(ResnetBlock(num_filters, name="Downsample", reshape=True, dtype=dtype))
        for i in range(num_blocks-1):
            self.sequence.add(ResnetBlock(num_filters, name="Resnet_%i" % i, reshape=False, dtype=dtype))
...
class FeatureAggregator(layers.Layer):
    def __init__(self, num_filters, dtype=None, name='FeatureAggregator'):
        ...
        self.upsample = layers.Conv2DTranspose(num_filters, (3, 3), (1,2), kernel_initializer='he_normal', padding='same', dtype=dtype)
