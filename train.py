...
def shape2(r, e, i, v, p, c, xyz):
    return tf.reshape(r, [64, 2650, 4])[:, 0:2648, :], tf.reshape(e, [4,4]), tf.reshape(i, [64]), tf.reshape(v, [-1, 5]), tf.reshape(p, [-1, 5]), tf.reshape(c, [-1, 5]), tf.reshape(xyz, [64, 2650, 3])
...
def fill_boxes2(input, nlz, indices, labels, label_classes):
    gathered = tf.gather(labels, indices, axis=1, batch_dims=1)
    dense_boxes = corners_from_centers(gathered)
    classes = tf.gather(label_classes, indices, axis=1, batch_dims=1)
    # classes = tf.where(tf.greater_equal(indices, 1), 1, 0)
    classes = classes[..., 0:2648]
    class_labels = tf.stack([classes, tf.cast(input['input_laser'][..., 4], tf.int32), tf.cast(nlz, tf.int32)], -1)
    box_labels = tf.concat([tf.expand_dims(tf.cast(classes, tf.float32), -1), dense_boxes, tf.expand_dims(tf.cast(indices, tf.float32), -1)], -1)
...
def transform2(r, e, i, vehicles, pedestrians, cyclists, xyz):
    polar_image = range_image_utils.compute_range_image_polar(tf.expand_dims(r[..., 0], 0), tf.expand_dims(e, 0), tf.reverse(tf.expand_dims(i, 0), [-1]))
    cloud = xyz[:, 0:2648, :]#range_image_utils.compute_range_image_cartesian(polar_image, tf.expand_dims(e, 0))
    flattened_cloud = tf.reshape(cloud[..., 0:2], [cloud.get_shape().num_elements() // 3, 2])
    labels_concat = tf.concat([[[0, 0, 0, 0, 0]], vehicles, pedestrians, cyclists], 0)
    class_label_mapping = tf.concat([[0], tf.ones(tf.shape(vehicles)[0:1], dtype=tf.int32), tf.fill(tf.shape(pedestrians)[0:1], 2), tf.fill(tf.shape(cyclists)[0:1], 3)], 0)
    bool_match = box_utils.is_within_box_2d(flattened_cloud, labels_concat)
    # Pad to add a zero-index indicating no box match
    # bool_match = tf.pad(bool_match, [[0, 0], [1, 0]], constant_values=False)
    indices = tf.reshape(tf.argmax(bool_match, axis=-1), r.get_shape()[:-1])
    ...
    mask = tf.where(mask, 1.0, 0.0)
    input_laser = tf.stack([azimuth, height, range, intensity, mask], -1)
    return {'input_laser': input_laser[..., 0:2648, :], 'input_xyz': cloud}, nlz[..., 0:2648], indices, labels_concat, class_label_mapping
...
datasets = []
for i in range(0, 20):
    ds = tf.data.experimental.load('/home/alex/dataset-drive/full_ds_sharded/shard_%d'%i, (
                                    ...
                                    tf.TensorSpec(shape=None, dtype=tf.float32),
                                    tf.TensorSpec(shape=None, dtype=tf.float32),
                                    tf.TensorSpec(shape=[64, 2650, 3], dtype=tf.float32)), compression='GZIP')
    datasets.append(ds)

tensor_ds = tf.data.experimental.sample_from_datasets(datasets).shuffle(500)
tensor_ds = tensor_ds.map(shape2, num_parallel_calls=32)
tensor_ds = tensor_ds.map(transform2, num_parallel_calls=32)
...
final_ds = tensor_ds.map(fill_boxes2, num_parallel_calls=tf.data.AUTOTUNE).with_options(options).prefetch(2)

validation_ds = tf.data.experimental.load('/home/alex/full_validation', (
                                            tf.TensorSpec(shape=[678400], dtype=tf.float32), 
                                            tf.TensorSpec(shape=[16], dtype=tf.float32), 
                                            tf.TensorSpec(shape=[64], dtype=tf.float32), 
                                            tf.TensorSpec(shape=None, dtype=tf.float32),
                                            ...
                                            tf.TensorSpec(shape=None, dtype=tf.float32),
                                            tf.TensorSpec(shape=[64, 2650, 3], dtype=tf.float32)), compression='GZIP')
validation_ds = validation_ds.map(shape2, num_parallel_calls=32)
validation_ds = validation_ds.map(transform2, num_parallel_calls=32)
validation_ds = validation_ds.apply(tf.data.experimental.dense_to_ragged_batch(batch_size=16, drop_remainder=True))
options = tf.data.Options()
options.experimental_distribute.auto_shard_policy = tf.data.experimental.AutoShardPolicy.DATA
validation_ds = validation_ds.map(fill_boxes2, num_parallel_calls=tf.data.AUTOTUNE).with_options(options).prefetch(2)
...
model.compile(optimizer=optimizer, loss={'classes': ClassLoss(object_classes=3, mixture_components=[1,1,1]), 'boxes': BoxLoss()})

# print(logs + '/images')
file_writer = tf.summary.create_file_writer(logs + '/images')
repeated_ds = final_ds.take(1).cache().repeat()
def evaluate_images(epoch, logs):
    for data, label in repeated_ds:
        ...
        test_label = label['classes'][..., 0:1]
        test_nlz = tf.cast(label['classes'][..., 2:] + 1, tf.float32) / 2.0
        with file_writer.as_default():
            tf.summary.image("Predict vehicle", test_pred[..., 0:1], step=epoch, max_outputs=4)
            tf.summary.image("Label vehicle", tf.where(test_label == 1, 1.0, 0.0), step=epoch, max_outputs=4)
